<template>
    <transition name="alert-fade">
        <div class="alert_state">
            <div class="content">
                <section class="state">
                    <aside><SvgIcon name="InvoiceSuccess" width="1.72" height="2.22" :color="mainColor"></SvgIcon></aside>
                ...
                <section class="btns" v-if="pageCont.isShowBtns ? false : true">
                    <aside class='go' @click="go" :style="{background: mainColor, 'box-shadow': `0px 0px 14px 0px ${colorRgb(mainColor)}`}">{{pageCont.btnCont}}</aside>
                    <aside class="back" @click="back" v-if="ua === 'wechat' && !ismp">{{pageCont.paySuccess ? pageCont.paySuccess : '返回公众号'}}</aside>
                </section>
            </div>
        </div>
    </transition>
