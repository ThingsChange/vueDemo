      ...
      <div class='layout-content'>
        <div class='layout-content-main'>
          <Content>
            ...
            <p>
              2:当created完成之后，他会去判断，instance 里面是否能含有 ‘el’选项，如果没有，它会调用vm.$mount(el)这个方法，然后执行下一步。
              如果有的话，直接执行下一步，紧接着会判断是否含有“template”这个选项，如果有的话，它会吧template解析成一个render function ,这是一个
              template编译的过程，结果是解析成了render 函数
              render(h){
                  return h('div',{},this.text)
              }
              render里面的h q其实就是Vue里面的createElement 方法，return 返回一个 createElement方法，其中要传3个参数，
              第一个参数就是创建的div标签；
              第二个参数传了一个对象，对象里面可以是我们组件上面的props，或者是事件之类的东西；
              第三个参数就是div标签里面的内容，这里我们指向了data里面的text。
              使用render函数的结果和我们之前使用template解析出来的结果是一样的，render函数是发生在 beforeMount 和mounted之间的额。
              这也就从侧面说明了，在beforeMount的时候，$el还只是我们写在html 里面的节点，然后到mounted的时候，它就吧渲染出来的内容挂在到了
