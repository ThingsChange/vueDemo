...
export default {
    ...
    return {
      tree:
        Object.keys(this.jsonObject).length === 0
          ? initData
          : JSON.parse(JSON.stringify(this.jsonObject)),
      initData,
      abc: true
    }
  },
  ...
  methods: {
    ...
    showJson() {
      ...
      //如果仅是一层
      if (
        ['string', 'number', 'boolean'].includes(this.tree.mapSet.type) &&
        this.tree.nodes &&
    ...
    getJson(tree, result) {
      ...
      tree.nodes &&
        tree.nodes.forEach(nodes => {
          this.getJson(
            nodes,
            result[tree.mapSet.label] || result[arrayIndexInArray]
          )
        })
    },
    /*
