    ...
    <Row type="flex" justify="space-between" style="margin-bottom:15px">
      <Col span="8" :style="indent">
        ...
        <Col span="22">
          <!-- :style="indentWith" -->
          <Input
            v-show="!parentTypeIsArray"
            :class="sameClass ? 'same-name' : ''"
            type="text"
            v-model="bindLabel"
            placeholder="请填写参数名"
          />
      ...
      <Col span="16">
        ...
        <Col span="2">
          <Col v-if="!isRoot" span="3" style="margin-right: 10px;">
            <Icon
              @click="handleMonitor(nowSet, parentSet, 'remove')"
              type="ios-close"
              class="ios-close-self"
            />
          </Col>
          <Col span="3">
            <Poptip
            ...
            >
              <div slot="content">
                <div v-if="isRoot" class="operation-pop-tip">
                  <span @click="handleMonitor(nowSet, parentSet, 'child')"
                    >添加子节点</span
                  >
                </div>
                <div v-else-if="!isObject" class="operation-pop-tip">
                  <span @click="handleMonitor(nowSet, parentSet, 'brother')"
                    >添加兄弟节点</span
                  >
                </div>
                <div v-else>
                  <div
                    v-for="(op, index) in operate.option"
                    :key="index"
                    class="operation-pop-tip"
                  >
                    <span @click="handleMonitor(nowSet, parentSet, op.value)">{{
                      op.label
                    }}</span>
                  </div>
                </div>
...
export default {
  ...
  watch: {
    ...
    bindLabel(val) {
      this.$emit('input', this.cengValue)
      let a = 0
      if (this.parentSet && this.parentSet.type === 'object') {
        //是对象的话，键值不能重复
    ...
    bindType(val, oldVal) {
      ...
      if (
        ['object', 'array'].includes(oldVal) &&
        !['object', 'array'].includes(val)
      ) {
