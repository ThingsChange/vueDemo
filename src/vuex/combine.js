  ...
  args.forEach(vuex => {
    ...
    if (isDev) {
      Object.keys(State).forEach(key => {
        if (state[key])
          throw new Error(`${key} : state should has the unique propties`)
        state[key] = State[key]
      })

      Object.keys(Getters).forEach(key => {
        if (getters[key])
          throw new Error(`${key} : getters should has the unique method`)
        getters[key] = Getters[key]
      })

      Object.keys(Actions).forEach(key => {
        if (actions[key])
          throw new Error(`${key} : actions should has the unique method`)
        actions[key] = Actions[key]
      })

      Object.keys(Mutations).forEach(key => {
        if (mutations[key])
          throw new Error(`${key} : mutations should has the unique method`)
