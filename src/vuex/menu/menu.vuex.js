...
Mutations[add_sub_meal_totalnum] = (state, selectFood) => {
    selectFood.meal.forEach((item) => {
        item.foodSetDetails.forEach((child) => {
            if (selectFood.count >= 1 && child.count && child.sku !== null && (child.sku >= 0)) {
    ...
    selectFood.meal.forEach((item) => {
        item.foodSetDetails.forEach((child) => {
            if (child.count && child.sku !== null && child.sku >= 0) {
                ...
                if (selectFood.count >= 1 && child.sku >= child.count) child.sku -= child.count;
                // child.count = 0;
            }
        });
    });
...
Mutations[init_selected_foods] = (state, shoppingCartInfo) => {
    ...
    state.selectedFoods = shoppingCartInfo && shoppingCartInfo.items && shoppingCartInfo.items.length > 0 ? ListToObjInList(shoppingCartInfo.items) : {};
    state.shopCartAcount = 0;
    if (Object.keys(state.selectedFoods).length) {
        state.shopCartAcount = Object.keys(state.selectedFoods)
            .map(item => state.selectedFoods[item].count * state.selectedFoods[item].strikePrice)
            .reduce((a, b = 0) => a + b);
    }
    state.exitSelectedFoods = state.selectedFoods;
    state.realTimeMarket = shoppingCartInfo && shoppingCartInfo.promotionName ? shoppingCartInfo.promotionName : '';
    state.takeoutPackingFee = shoppingCartInfo && shoppingCartInfo.takeoutPackingFee ? shoppingCartInfo.takeoutPackingFee : 0;
...
Mutations[pick_delivery_money] = (state, money) => {
    ...
    state.pickDeliveryMoney = findPrice.length > 1 ? Number(`${findPrice[0]}.${findPrice[1]}`) : Number(findPrice[0]);
};
// 设置就餐人数
...
Mutations[set_vip_pay_type] = (state, vipPrice) => {
    ...
    const allVipAcount = (state.selectedCard.pointBalance * state.selectedCard.pointExchangeRate) + state.selectedCard.cardBalance;
    if (allVipAcount < state.confirmOrderAcount && state.selectedCard.cardStatus === 10) state.selectedCard.cardNotCanUsingNotes = '余额不足';
};
...
Mutations[set_vip_cards_status] = (state) => {
    state.vipCards.forEach((card) => {
        const allVipAcount = (card.pointBalance * card.pointExchangeRate) + card.cardBalance;
        if (allVipAcount < state.confirmOrderAcount && card.cardStatus === 10) card.cardNotCanUsingNotes = '余额不足';
    });
};
...
Mutations[selected_promotion_wrappers] = (state, pro) => {
    ...
    if (curItem && curItem.length > 0) {
        const price = curItem.map(acountItem => acountItem.promotionAmount).reduce((a, b = 0) => a + b);
        state.discountCount = price;
    }
    if (promotion.vipInfos && promotion.vipInfos.length > 0) {
        const price = promotion.vipInfos.map(acountItem => acountItem.promotionAmount).reduce((a, b = 0) => a + b);
...
Mutations[init_insufficient_inventory] = (state, inventoryMessages) => {
    state.inventoryMessages = inventoryMessages && inventoryMessages.length > 0 ? inventoryMessages : [];
    if (inventoryMessages && inventoryMessages.length > 0) state.isShowInsufficientInventory = true;
};
...
Mutations[set_again_food] = (state, shoppingCartInfo) => {
    ...
    state.realTimeMarket = shoppingCartInfo && shoppingCartInfo.promotionName ? shoppingCartInfo.promotionName : '';
    state.takeoutPackingFee = shoppingCartInfo && shoppingCartInfo.takeoutPackingFee ? shoppingCartInfo.takeoutPackingFee : 0;
    state.inventoryMessages = shoppingCartInfo && shoppingCartInfo.inventoryMessages && shoppingCartInfo.inventoryMessages.length > 0 ? shoppingCartInfo.inventoryMessages : [];
    if (shoppingCartInfo.inventoryMessages && shoppingCartInfo.inventoryMessages.length > 0) state.isShowInsufficientInventory = true;
...
Mutations[init_store_set_value] = (state, storeSetValue) => {
    ...
    state.storeSetList = (storeSetValue && storeSetValue.saveMoneySetResList) || [];
    state.storeSetList.forEach((item, index) => {
        const obj = item;
        obj.index = index;
        return obj;
    });
    state.selectedStoreSet = (state.storeSetList.filter(item => item.recommendSaveMoneyPolicy).length && state.storeSetList.filter(item => item.recommendSaveMoneyPolicy)[0]) || {};
};
...
Mutations[set_checked_gift] = (state, obj) => {
    state.foodPromotions[obj.index].optionalFoodInfo.items.forEach((item, subIndex) => {
